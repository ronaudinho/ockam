...
use types::{CredentialRequest, Enroller, EnrollerInfo, MemberCredential};
            ...
            Some(Method::Post) => match req.path_segments::<2>().as_slice() {
                ["enroll"] => {
                    ...
                    if let Some(data) = self.store.get(ENROLLER, from.key_id()).await? {
                        ...
                        self.store
                            .set(crq.member().as_str(), DIRECT.to_string(), vec)
                            .await?;
        ...
        let res = match req.method() {
            Some(Method::Post) => match req.path_segments::<2>().as_slice() {
                ["register"] => {
                    ...
                    self.store
                        .set(ENROLLER, e.enroller().as_str().into(), b)
                        .await?;
...
impl Client {
    ...
    pub async fn register(&mut self, req: &Enroller<'_>) -> Result<()> {
        ...
        self.buf = self
            .request("register", "enroller_registration", &req)
            .await?;
