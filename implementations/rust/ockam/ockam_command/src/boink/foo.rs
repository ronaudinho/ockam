...
impl FooCommand {
    pub fn run(opts: CommandGlobalOpts, cmd: FooCommand) {
        let cfg = &opts.config;
        ...
        connect_to(port, (opts, cmd), foo);
    }
}

async fn foo(
    ctx: ockam::Context,
    (opts, cmd): (CommandGlobalOpts, FooCommand),
    ...
    let response: Vec<u8> = ctx
        .send_and_receive(route, api::boink::foo(cmd)?)
        .await
